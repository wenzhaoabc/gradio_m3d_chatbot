...
class m3d_chatbot(Component):
    ...
    def __init__(
        ...
        *,
        ...
        Parameters:
            ...
            placeholder: a placeholder message to display in the chatbot when it is empty. Centered vertically and horizontally in the m3d_chatbot. Supports Markdown and HTML. If None, no placeholder is displayed.
            examples: A list of example messages to display in the chatbot before any user/assistant messages are shown. Each example should be a dictionary with an optional "text" key representing the message that should be populated in the m3d_chatbot when clicked, an optional "files" key, whose value should be a list of files to populate in the m3d_chatbot, an optional "icon" key, whose value should be a filepath or URL to an image to display in the example box, and an optional "display_text" key, whose value should be the text to display in the example box. If "display_text" is not provided, the value of "text" will be displayed.
    ...
    def _preprocess_content(
        ...
        if chat_message is None:
            ...
            if component is not None:
                ...
                raise ValueError(
                    f"Invalid component for m3d_chatbot component: {chat_message.component}"
                )
        else:
            raise ValueError(f"Invalid message for m3d_chatbot component: {chat_message}")
    ...
    def _create_file_message(chat_message, filepath):
        ...
        return FileMessage(
            ...
            alt_text=m3d_chatbot._get_alt_text(chat_message),
        )
    ...
    def _postprocess_message_messages(
        ...
        if isinstance(message, dict):
            ...
            raise Error(
                f"Invalid message for m3d_chatbot component: {message}", visible=False
            )

...
const preview_languages = ["mermaid", "html"];
...
const renderer: Partial<Omit<Renderer, "constructor" | "options">> = {
	code(
		...
		if (preview_languages.includes(lang)) {
			if (lang === "mermaid") {
				html_str += `<div class="mermaid">${original_code}</div>`;
			}
			if (lang === "html") {
				html_str += `<iframe sandbox="allow-forms allow-modals allow-scripts" style="width: 100%; margin-top:6px; border-radius: 6px; border: var(--border-color-primary); background-color: var(--background-fill-primary);" srcdoc="${original_code.replace(/\s+/g, ' ')}"></iframe>`;
			}
		}
		return html_str;
	},
